...
// Client IP for the ip_hash bucket. Only the digest is ever persisted (PII rule).
...
reports.post("/", strictRateLimit, async (c) => {
  ...
  const dup = await findDuplicateGroup(db, input.service_id, input.district_id, input.description);
